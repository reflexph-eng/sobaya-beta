...
import { computePaymentBalance, computePaymentStatus, expectedMonthlyPayment, isActiveContract } from "@/services/business-rules";
...
export function PaymentForm({ payment, contracts, loading, onSubmit, onCancel }: {
  ...
  const expectedAmount = expectedMonthlyPayment(selectedContract);
  ...
  useEffect(() => {
    if (payment) {
      ...
      const defaultExpectedAmount = expectedMonthlyPayment(defaultContract);
      setValues({
        ...initialValues,
  ...
  function setContract(contractId: string) {
    ...
    const amount = expectedMonthlyPayment(contract);
    setValues((current) => ({
      ...current,
      contractId,
      amount,
      status: computePaymentStatus(amount, amount)
    }));
  }
  ...
  function setAmount(amount: number) {
    setValues((current) => ({
      ...current,
      amount,
      status: computePaymentStatus(amount, expectedMonthlyPayment(contracts.find((contract) => contract.id === current.contractId)))
    }));
  }
    ...
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      <label className="grid gap-2 text-sm font-medium text-sobaya-ink">
        ...
        <select
          ...
          className="h-11 rounded-xl border border-sobaya-border bg-white px-3 text-sm outline-none focus:border-sobaya-ink"
        >
      ...
      {selectedContract ? (
        <div className="rounded-xl border border-sobaya-border bg-sobaya-soft/50 px-4 py-3 text-sm text-sobaya-ink">
          <p className="font-medium">Montant attendu : {money(expectedAmount)}</p>
          <p className="text-sobaya-muted">Loyer {money(Number(selectedContract.monthlyRent) || 0)} + charges {money(Number(selectedContract.charges) || 0)} · {selectedContract.tenantName} · {selectedContract.propertyName}</p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <FormField label="Montant payé (FCFA)" required help="Le montant attendu est prérempli. Modifiez-le seulement en cas d'avance ou de paiement partiel.">
          <input type="number" min={0} value={values.amount} onChange={(event) => setAmount(Number(event.target.value))} placeholder="Ex : 250000" className="h-11 rounded-xl border border-sobaya-border bg-white px-3 text-sm outline-none focus:border-sobaya-ink" />
        </FormField>
        <label className="grid gap-2 text-sm font-medium text-sobaya-ink">
          Mode de paiement *
        ...
        <FormField label="Statut du paiement" help="Calculé automatiquement à partir du montant attendu et du montant payé.">
