            ...
            {contracts.map((contract) => (
              <div key={contract.id} className="rounded-2xl border border-sobaya-border p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <StatusBadge>{statusLabels[contract.status] ?? contract.status}</StatusBadge>
                </div>
                <div className="mt-4 grid gap-2 text-sm text-sobaya-muted sm:grid-cols-3">
                  <p>Loyer : <span className="text-sobaya-ink">{money(contract.monthlyRent)}</span></p>
                  <p>Charges : <span className="text-sobaya-ink">{money(contract.charges)}</span></p>
                  <p>Échéance : <span className="text-sobaya-ink">jour {contract.dueDay}</span></p>
                </div>
